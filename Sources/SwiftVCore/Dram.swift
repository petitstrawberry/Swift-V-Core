...
public struct Dram: Device {
    public let startAddr: UInt32 = 0x8000_0000
    public let endAddr: UInt32 = 0x8000_0000 + kMemorySize - 1
    ...
    public mutating func write8(addr: UInt32, data: UInt8) {
        mem[Int(addr - startAddr)] = data
    }

    public mutating func write16(addr: UInt32, data: UInt16) {
        let intAddr = Int(addr - startAddr)
        mem[intAddr] = UInt8(data & 0xFF)
        mem[intAddr + 1] = UInt8((data >> 8) & 0xFF)
    }

    public mutating func write32(addr: UInt32, data: UInt32) {
        let intAddr = Int(addr - startAddr)
        mem[intAddr] = UInt8(data & 0xFF)
        mem[intAddr + 1] = UInt8((data >> 8) & 0xFF)
